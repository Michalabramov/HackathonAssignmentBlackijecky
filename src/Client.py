...
class Client:
    ...
    def start(self):
        """
        Main client loop. Listens for UDP offers, connects to servers via TCP,
        and manages game sessions.
        """
        print(f"Client started, listening for offer requests...")
        
        while True:
            # 1. Wait and listen for UDP broadcast offers from servers
            ...
            if offer_data:
                ...
                # 2. Establish TCP connection and execute the game logic
                try:
    ...
    def wait_for_offer(self):
        """
        Listens on a hardcoded UDP port for server advertisements.
        Returns server connection details upon receiving a valid offer packet.
        """
        with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as udp_sock:
            # Enable port reuse to allow multiple clients on the same machine
            udp_sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
            try:
                # Specific option for Linux/Unix environments if needed
                udp_sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEPORT, 1)
            except AttributeError:
                pass 
                
            # Bind to the broadcast port defined in the requirements
            udp_sock.bind(('', Constants.Constants.UDP_PORT))
            
            while True:
                data, addr = udp_sock.recvfrom(1024)
                # Use PacketHandler to validate the magic cookie and type
                result = PacketHandler.PacketHandler.unpack_offer(data)
                if result:
                    port, name = result
    ...
    def play_game(self, ip, port, rounds):
        """
        Handles TCP communication, sends the initial game request,
        and loops through the specified number of rounds.
        """
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as tcp_sock:
            tcp_sock.connect((ip, port))
            
            # Send the initial request packet containing team name and round count
            request_packet = PacketHandler.PacketHandler.pack_request(rounds, self.team_name)
            ...
            for r in range(rounds):
                ...
                if result == Constants.Constants.WIN:
                    current_wins += 1
                    self.wins += 1
                self.total_rounds += 1
    ...
    def run_round(self, sock):
        ...
        while True:
            ...
            res, rank, suit = PacketHandler.PacketHandler.unpack_payload_server(data)
            
            # Check if the round is still active
            if res == Constants.Constants.ROUND_NOT_OVER:
                # Calculate card value using the game logic rules
                card_val = BlackjackGame.BlackjackGame.get_card_value(rank)
                ...
                sock.sendall(PacketHandler.PacketHandler.pack_payload_client(decision))
                ...
                if res == Constants.Constants.WIN: print("Result: YOU WIN!")
                elif res == Constants.Constants.LOSS: print("Result: YOU LOSE!")
                else: print("Result: IT'S A TIE!")
                return res

if __name__ == "__main__":
    # Create a client instance with a creative name and start listening
    client = Client("JokerClient")
    client.start()